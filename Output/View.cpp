...
View::View(QWidget *parent, const std::shared_ptr<Patronus::Camera> & cam, const std::string & shaderId )
    : QOpenGLWidget(parent), _VAO( 0 ), _shaderId( shaderId )
{
    if ( cam == nullptr )
...
void View::toImageFile_color( const std::string & fileName ){
    ...
    repaint();
    grabFramebuffer().save(QString(fileName.c_str()));
    //glReadPixels(0, 0, width(), height(), GL_RGBA, GL_UNSIGNED_BYTE, image.bits());

    //Utils::logOpenGLError();

    //image.mirrored().save(QString(fileName.c_str()));
    //Utils::logOpenGLError();
}
...
void View::toImageFile_depth( const std::string & fileName ) {
    ...
    _shaderId = Lumos::Shader::depth_shader_id;
    repaint();
    toImageFile_color(fileName);
    _shaderId = temp;
